...
export default function EnhancedRoadmapTracker({ roadmap, completedPhases = [], onPhaseComplete }) {
  ...
  const addToQueue = (node) => {
    if (!courseQueue.find(i => i.id === node.id)) {
      setCourseQueue([...courseQueue, node]);
      toast.success(`Added "${node.label}" to course queue`);
    } else {
      toast.info(`"${node.label}" is already in the queue`);
    }
  };

  const removeFromQueue = (nodeId) => {
    setCourseQueue(courseQueue.filter(i => i.id !== nodeId));
  };

  const createCustomCourse = () => {
    if (courseQueue.length === 0) {
      toast.error("Queue is empty. Add topics from the roadmap first.");
      return;
    }
    
    // Save queue to session storage for the learning page to pick up
    sessionStorage.setItem('customCourseQueue', JSON.stringify(courseQueue));
    toast.success("Creating custom course from queue...");
    router.push('/integrated-learning?mode=custom');
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                variant={view === "flowchart" ? "default" : "outline"}
                size="sm"
        ...
        <CardContent className="p-0">
          {view === "flowchart" ? (
            <div className="h-[600px] w-full border-t border-slate-800">
              <RoadmapFlowchart
                ...
                roadmapData={(() => {
                  const nodes = [
                    {
                      ...
                      type: 'start',
                      position: { x: 0, y: 0 },
                      description: 'Begin your learning path here'
                    }
                  ];
                  const edges = [];
                  ...
                  activeRoadmap.phases?.forEach((phase, idx) => {
                    ...
                    // 1. Week/Phase Node (The Spine)
                    nodes.push({
                      id: weekId,
                      label: `Week ${idx + 1}: ${phase.name}`, 
                      type: 'core', // Special styling for spine
                      position: { x: 0, y: 0 },
                      description: `Focus for Week ${idx + 1}. Contains ${phase.topics?.length || 0} topics and ${phase.projects?.length || 0} projects.`,
                      status: completedPhases.includes(idx) ? 'completed' : 'pending',
                      resources: {
                        free: phase.resources || [
                          { title: `${phase.name} Overview`, url: "#" },
                          { title: "Documentation", url: "#" }
                        ],
                        videos: [
                          { title: `${phase.name} Crash Course`, url: "#" }
                        ]
                      }
                    });

                    // Connect to previous week (Spine connection)
                    edges.push({
                      id: `e-${lastPhaseId}-${weekId}`,
                      source: lastPhaseId,
                      target: weekId,
                      type: 'smoothstep',
                      animated: true,
                      style: { strokeWidth: 3, stroke: '#3b82f6' } // Thicker blue line for spine
                    });

                    // 2. Topic Nodes (Branching out)
                    phase.topics?.forEach((topic, tIdx) => {
                      ...
                      nodes.push({
                        ...
                        position: { x: 0, y: 0 },
                        status: 'pending',
                        description: `Detailed learning path for ${topic}. Master this concept to progress.`,
                        resources: {
                          free: [
                            { title: `${topic} Guide`, url: "#" },
                            { title: "MDN Web Docs", url: "https://developer.mozilla.org" }
                          ],
                          videos: [
                            { title: `Learn ${topic} in 10 Minutes`, url: "#" },
                            { title: `${topic} Full Course`, url: "#" }
                          ]
                        }
                      });
      ...
      {courseQueue.length > 0 && (
        ...
        >
          <Card className="shadow-2xl border-t-4 border-t-blue-500 bg-slate-950/95 backdrop-blur border-slate-800">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center justify-between text-lg">
                ...
                <Badge variant="secondary">{courseQueue.length} items</Badge>
              </CardTitle>
              <CardDescription>Custom learning queue</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="max-h-48 overflow-y-auto space-y-2 pr-2">
                {courseQueue.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between p-2 rounded bg-slate-900 border border-slate-800 text-sm">
